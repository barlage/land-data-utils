program data_regrid
  ...
  character*50     :: suffix        ! cci NC fix suffix
  character*50     :: out_name      ! cci output file name
...
! ---- Get values from  CCI-C96_bilinear_wts.nc  ------------
! ---- destination_locs=n_b,  weight_locs=n_s ----------------

  integer, parameter           :: destination_locs      = 4718592
  integer, parameter           :: weight_locs           = 11009332

  integer, parameter           :: destination_lats      = 1536
  integer, parameter           :: destination_lons      = 3072
...
! Define 2D array
  real   , dimension(destination_lons, destination_lats) :: soilm
  real   , dimension(destination_lons, destination_lats) :: error
  real   , dimension(destination_lons, destination_lats) :: lon2d
  real   , dimension(destination_lons, destination_lats) :: lat2d

! Define 1D lat and lon
  real   , dimension(destination_lons) :: lon1d
  real   , dimension(destination_lats) :: lat1d
  ...
  namelist/regrid_cci_nml/cgrid, source_path, weights_path, destination_path

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! read namelist
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

 inquire(file='regrid_cci.nml', exist=file_exists)
...
open (action='read', file='regrid_cci.nml', iostat=ierr, newunit=io)
read (nml=regrid_cci_nml, iostat=ierr, unit=io)
close (io)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  suffix=yyyy//mm//dd//'000000-fv08.1.nc'
  out_name='cci_sm_'//yyyy//mm//dd//'_'//trim(cgrid)//'.nc'
  write(*,*) current_date
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Read input, weight, and output data path
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  source_filename=trim(source_path)//'ESACCI-SOILMOISTURE-L3S-SSMV-COMBINED-'//trim(suffix)

  status = nf90_open(trim(source_filename), NF90_NOWRITE, ncid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_inq_varid(ncid, "sm", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_get_var(ncid, varid , source_input, start = (/1,1,1/), count = (/source_lons,source_lats,1/))
    if (status /= nf90_noerr) call handle_err(status)

   status = nf90_inq_varid(ncid, "sm_uncertainty", varid)
    if (status /= nf90_noerr) call handle_err(status)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  weights_filename=trim(weights_path)//'CCI-'//trim(cgrid)//'_bilinear_wts.nc'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do iwt = 1, destination_locs
    ...
    soilMoisture(iwt) = soilMoisture(iwt)/soilMoistureWT(iwt)
    end if

    if(uncertaintyWT(iwt) > 0.0 ) then
    uncertainty(iwt) = uncertainty(iwt)/uncertaintyWT(iwt)
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i = 1, destination_lons
    itotal=itotal+1
    soilm(i,j) = soilMoisture(itotal)
    error(i,j) = uncertainty(itotal)
...
! Define variables in the file.
  ...
  status = nf90_def_var(ncid, "soilMoisture", NF90_FLOAT, (/dim_id_i,dim_id_j, dim_id_t/), varid)
    if (status /= nf90_noerr) call handle_err(status)
  ...
  status = nf90_def_var(ncid, "uncertainty", NF90_FLOAT, (/dim_id_i, dim_id_j, dim_id_t/), varid)
    if (status /= nf90_noerr) call handle_err(status)
...
! Write variables in the file.
  ...
  status = nf90_inq_varid(ncid, "soilMoisture", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_var(ncid, varid , soilm, start = (/1,1,1/), count = (/destination_lons,destination_lats,1/))
    if (status /= nf90_noerr) call handle_err(status)
 
  status = nf90_inq_varid(ncid, "uncertainty", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_var(ncid, varid , error, start = (/1,1,1/), count = (/destination_lons, destination_lats,1/))
    if (status /= nf90_noerr) call handle_err(status)
