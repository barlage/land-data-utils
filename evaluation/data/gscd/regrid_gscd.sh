...
#SBATCH --job-name=regrid_gscd
#SBATCH -o regrid_gscd.out
#
...
subset_name="conus"
data_source="gscd"
data_source_file="/scratch4/NCEPDEV/land/data/evaluation/GSCD/original/GSCD_v2.0_postprocess.nc"
data_destination_directory="/scratch4/NCEPDEV/land/data/evaluation/GSCD/"
...
# create separate scrip files, one for precip, one for others

data_scrip_file=$data_source"_SCRIP.nc"
precip_scrip_file=$data_source"_precip_SCRIP.nc"

# create the ncl parameter file

echo "data_scrip_file = $data_scrip_file" > regrid_parameter_assignment
echo "precip_scrip_file = $precip_scrip_file" >> regrid_parameter_assignment
...
# create weights file for precip

weights_filename_precip=$data_source"-"$grid_string"_"$interpolation_method"_wts_precip.nc"
echo "Creating weights file: "$weights_filename_precip

srun -n $SLURM_NTASKS time ESMF_RegridWeightGen --netcdf4 --ignore_degenerate \
       --source $precip_scrip_file \
       --destination $destination_scrip_file \
       --weight $output_path$weights_filename_precip --method $interpolation_method \
       --ignore_unmapped

# create weights for all other variables
...
echo "weights_filename_precip = $output_path$weights_filename_precip" >> regrid_parameter_assignment
