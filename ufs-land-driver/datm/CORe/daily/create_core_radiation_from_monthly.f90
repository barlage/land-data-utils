program create_core_radiation_from_monthly

! create hourly SWdn from monthly three-hourly files
!   use cosz weighting to diaggreggate three-hourly average
!
! since the CORe data are forward averages, the first time is missing in the first file
!   just fill with hour 1
!   ncap2 -s 'downward_solar(0,:,:)=downward_solar(1,:,:)' CORe_forcing_1980-01-01.nc CORe_forcing_1980-01-01.nc

use netcdf
implicit none

character*128 :: yyyy_beg_input
character*128 :: yyyy_end_input
character*128 ::   mm_beg_input
character*128 ::   mm_end_input
...
integer :: iyyyy,imm,itime,iday,ihour,i30,ilat,ilon
...
integer :: number_days
integer :: next_mm,next_yyyy,next_dd,prev_time
real*8  :: time_period_begin

character*256 :: filename
logical       :: file_exists
character*19  :: utc_date

real*8, allocatable :: time_monthly(:)
real*8, allocatable :: time_daily(:)
real  , allocatable :: latitude(:)
real  , allocatable :: longitude(:)
real  , allocatable :: downward_solar_monthly(:,:,:)
real  , allocatable :: downward_solar_daily(:,:,:)
real  , allocatable :: cosz(:,:,:)
real  , allocatable :: cosz1(:,:), cosz2(:,:), cosz3(:,:), coszsum(:,:)

call getarg(1,yyyy_beg_input)
call getarg(2,  mm_beg_input)
call getarg(3,yyyy_end_input)
call getarg(4,  mm_end_input)

read(yyyy_beg_input,'(i4)') yyyy_beg
read(  mm_beg_input,'(i2)')   mm_beg
read(yyyy_end_input,'(i4)') yyyy_end
read(  mm_end_input,'(i2)')   mm_end
...
! read the current month and next month (first time only)
  ...
  allocate(downward_solar_monthly(number_lons,number_lats,0:number_times))  ! add one to time for the beginning of next month
  ...
  status = nf90_inq_varid(ncid, "downward_solar", varid)
    if(status /= nf90_noerr) call handle_err(status, "problem inquire downward_solar")
  status = nf90_get_var(ncid, varid , downward_solar_monthly(:,:,1:number_times))
    if (status /= nf90_noerr) call handle_err(status, "problem getting downward_solar")

  status = nf90_close(ncid)
  
! now loop through the days to create hourly values

  allocate(downward_solar_daily(number_lons,number_lats,24))
  allocate(cosz(number_lons,number_lats,7))
  allocate(cosz1(number_lons,number_lats))
  allocate(cosz2(number_lons,number_lats))
  allocate(cosz3(number_lons,number_lats))
  allocate(coszsum(number_lons,number_lats))
...
! get cosz for times to get average

    do i30  = 1, 7

      call date_from_since("1970-01-01 00:00:00", time_period_begin+1800.0*(i30-1), utc_date)
!      print*, "Starting zenith calc: ", trim(utc_date)

      do ilat = 1, number_lats
      do ilon = 1, number_lons

        call calc_cosine_zenith(utc_date,latitude(ilat),longitude(ilon),cosz(ilon,ilat,i30))
      
      end do
      end do

      where(cosz < 0.0) cosz = 0.0

    end do

    cosz1 = (cosz(:,:,1) + cosz(:,:,2) + cosz(:,:,3)) / 3.0 ! average cosz for first hour
    cosz2 = (cosz(:,:,3) + cosz(:,:,4) + cosz(:,:,5)) / 3.0 ! average cosz for second hour
    cosz3 = (cosz(:,:,5) + cosz(:,:,6) + cosz(:,:,7)) / 3.0 ! average cosz for third hour
      
    coszsum = cosz1 + cosz2 + cosz3

    where(coszsum > 0.0) cosz1 = cosz1 / coszsum
    where(coszsum > 0.0) cosz2 = cosz2 / coszsum
    where(coszsum > 0.0) cosz3 = cosz3 / coszsum

    downward_solar_daily(:,:,(ihour-1)*3+1) = 3.0 * cosz1 * downward_solar_monthly(:,:,prev_time)
    downward_solar_daily(:,:,(ihour-1)*3+2) = 3.0 * cosz2 * downward_solar_monthly(:,:,prev_time)
    downward_solar_daily(:,:,(ihour-1)*3+3) = 3.0 * cosz3 * downward_solar_monthly(:,:,prev_time)

  end do  ! three hourly loop

! open the existing output filename and netcdf file
  ...
  status = nf90_inq_varid(ncid, "downward_solar", varid)
    if(status /= nf90_noerr) call handle_err(status, "problem inquire downward_solar")
  status = nf90_put_var(ncid, varid , downward_solar_daily(:,:,1:23), start=(/1,1,2/), count=(/number_lons,number_lats,23/) )
    if (status /= nf90_noerr) call handle_err(status, "problem putting downward_solar")
  ...
  write(filename,'(a13,i4,a1,i2.2,a1,i2.2,a3)') "CORe_forcing_", next_yyyy, "-", next_mm, "-", next_dd, ".nc"
  filename = trim(daily_path)//trim(filename)

  inquire(file=filename, exist=file_exists)
  if(file_exists) then   ! prevent write for final time

    print *, "Writing: ", trim(filename)

    status = nf90_open(filename, NF90_WRITE, ncid)
      if (status /= nf90_noerr) call handle_err(status,"problem opening 2nd output file")
  
    status = nf90_inq_varid(ncid, "downward_solar", varid)
      if(status /= nf90_noerr) call handle_err(status, "problem inquire downward_solar")
    status = nf90_put_var(ncid, varid , downward_solar_daily(:,:,24), start=(/1,1,1/), count=(/number_lons,number_lats,1/) )
      if (status /= nf90_noerr) call handle_err(status, "problem putting 2nd downward_solar")
  ...
  deallocate(downward_solar_monthly)
  deallocate(downward_solar_daily)
  deallocate(cosz)
  deallocate(cosz1)
  deallocate(cosz2)
  deallocate(cosz3)
