...
error = nf90_def_var(ncid_out, "vegetation_type", NF90_BYTE, (/ncdim_idim,ncdim_jdim,ncdim_time/), varid)
 call netcdf_err(error, "defining variable: vegetation_type")
...
error = nf90_inq_varid(ncid_in, 'surface_type', varid)
 call netcdf_err(error, 'inquire surface_type variable' )

error = nf90_get_var(ncid_in, varid, surface_type_in)
...
error = nf90_inq_varid(ncid_out, 'vegetation_type', varid)
 call netcdf_err(error, 'inquire vegetation_type variable' )

error = nf90_put_var(ncid_out, varid, surface_type_out)
